...
function ajaxError($error_msg='', $http_error_code=400) {
    setAjaxError($error_msg,$http_error_code);
}
...
function setAjaxError($error_msg='',$http_code=400) {

    if($error_msg == '' && $http_code == 403 )
        $error_msg = 'You have no permissions.';

    http_response_code ($http_code);
    header("Error-Msg: ".$error_msg);
    echo json_encode(array('status' => 0, 'error' => $error_msg));
    die;

}

function setAjaxComplete($data='',$exit_script = true) {
    $out = array('complete' => 1, 'status' => 1);
    if(is_array($data)) { $data = array('data' => $data); $out = array_merge($out,$data); }
...
if (!function_exists('Redir')) {
    ...
    function Redir($link)
    {
        ...
        //    if(ACES_IN_ADMIN) echo "<meta http-equiv=\"REFRESH\" content=\"0;url=/admin/$link\"> ";
...
function Redirect($link){
    ...
    $_SERVER['SCRIPT_NAME'] = str_replace(".php", '', $_SERVER['SCRIPT_NAME']);

    if(in_array($link, array('/admin/login', '/login', '/signin', '/lock', '/admin/lock') )) {
        //if($link == '/admin/login' || $link == '/login' || $link == 'login' || $link == 'signin') {
        if(!in_array($_SERVER['SCRIPT_NAME'], array('lock.php', 'signin.php', 'logout.php', '/admin/logout.php', '/admin/lock.php' ) ))
            $link = $link . "?ref=".$_SERVER['SCRIPT_NAME']."?".$_SERVER['QUERY_STRING'];
    }
...
function Redir2($link){

    //$_SERVER['SCRIPT_NAME'] = str_replace(".php", '', $_SERVER['SCRIPT_NAME']);

    if(in_array($link, array('/admin/login', '/login', '/signin', '/lock', '/admin/lock') )) {
        //if($link == '/admin/login' || $link == '/login' || $link == 'login' || $link == 'signin') {
        if(!in_array($_SERVER['SCRIPT_NAME'], array('lock', 'signin', 'logout', '/admin/logout', '/admin/lock' ) ))
            $link = $link . "?ref=".$_SERVER['SCRIPT_NAME']."?".$_SERVER['QUERY_STRING'];
    }
...
function country_code_by_name( $code ){
    ...
    if( $code == 'US' ) return 'United States'; //United States of America
