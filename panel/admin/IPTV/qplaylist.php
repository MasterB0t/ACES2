...
if(isset($_GET['file_id'])) {
    ...
    if($stream['stream'] == 0 ) {
       $r=$db->query("SELECT url FROM iptv_channels_sources WHERE chan_id = $stream_id AND enable = 1 ORDER BY priority LIMIT 1 ");
...
GROUP BY s.id ORDER BY access ASC LIMIT 1");
    ...
    $r_server = $db->query("SELECT id,address,port,sport,api_token FROM iptv_servers WHERE id = '{$streaming['server_id']}'");
    if (!$server = $r_server->fetch_assoc())
        notFound();
...
VALUES('$stream_id',-1,'{$server['id']}','{$server['address']}','$token','{$_SERVER['REMOTE_ADDR']}','{$_SERVER['HTTP_USER_AGENT']}','MPEGTS',NOW() + interval 60 SECOND,NOW(),NOW())");
    ...
    $PRTC = 'http:'; $PORT = $server['port'];
    if( isset($_SERVER['HTTPS']) && (int)$server['sport'] > 0 ) { $PRTC = 'https:'; $PORT = $server['sport']; }
