...
if($config['iptv']['api_cache'] ) {
    ...
    $Cache = new Cache("/home/aces/cache/vods_categories_$cache_filename", $config['iptv']['api_cache'],$config['iptv']['api_cache_compressed'] );
    ...
    if ($data = $Cache->get(true)) {
        ...
        if(!$Cache->isExpired()) {
            ...
            if(is_file("/home/aces/run/build_cache_vod_cats".$cache_filename)) {
                exit;
            }

            $building_cache = 1;
            //touch("/home/aces/run/build_cache_vod_cats" . $cache_filename);
...
$r=$DB->query("SELECT cat.name as name,cat.id as id FROM iptv_ondemand o 
        ...
        RIGHT JOIN iptv_stream_categories cat on cat.id = i.category_id
        WHERE o.status = 1 AND o.type = 'movies' $SQL_ADULTS AND p.bouquet_id IN ('{$ACCOUNT['bouquets']}')
        GROUP BY cat.id ORDER BY cat.m_ordering
");
...
if(!$building_cache)
    ...
    $a=1;
    //unlink("/home/aces/run/build_cache_vod_cats" . $cache_filename);
}
...
if($config['iptv']['api_cache'])
    $Cache->saveit(json_encode($categories));
    //SaveCache("/home/aces/cache/vods_categories_$cache_filename",json_encode($categories));
