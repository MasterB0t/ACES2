...
$r=$DB->query("SELECT stream_profile,source_type,ondemand,type FROM iptv_channels WHERE id = $StreamID ");
...
while(true) {
        ...
        if(true) {
            ...
            while($y<30) {
                ...
                if(!empty($chunk)) {
                    ...
                    $DB->query("UPDATE iptv_streaming SET status = 1, source_id = '{$s['id']}', 
                          video_codec = '{$stream_v_info['streams'][0]['codec_name']}', audio_codec = '{$stream_a_info['streams'][0]['codec_name']}', 
                          `bitrate` = '$bitrate', info = '$s_info', reconnected = reconnected + 1, connected_datetime=NOW() 
            ...
            while($CONNECTED) {
                ...
                if( ($CurrentPlaylistTime +60)  < time() ) {
                    //logfile('STREAM STOP, NOT RECEIVING DATA.');
                    break;
                }
            ...
            $aces_sinfo_pid=null;
            $aces_sinfo_pid=shell_exec("php /home/aces/bin/aces_stream_info.php $StreamID- > /dev/null 2>&1 & echo $!" );
            $DB->close();
            exec ($proxy.$cmd);
    ...
    if($skipped >= count($SOURCES) ) {
        ...
        if($channel_info['ondemand']) {
            ...
            exec(" rm -rf /home/aces/stream_tmp/$StreamID-*");
            logfile("ON DEMAND NO CLIENT, GOING TO STAND BY", L_DEBUG);
            $StreamStats->setStandBy();
            die;
        }
