...
if(is_file("/home/aces/run/aces_import_from_xc.pid")) {
    ...
    if(posix_getpgid($PID)) die("Another importer process is running please wait until it finished.");
} else {
...
echo "\n\n{$RED}This process could take several minutes. Please wait until it finished.\n{$WHITE}";



echo "\n\nImporting catergories...\n";
$r=$DB_XC->query("SELECT * FROM `streams_categories` ");
...
//    $category_id = str_replace("[","",$row['category_id']);
//    $category_id = str_replace("]","",$category_id);
//    $category_id = str_replace(",","",$category_id);

    $category_id = json_decode($row['category_id'])[0];
...
//    if(file_put_contents("/home/aces/panel/logos/$img", file_get_contents($m[0][0]))) $logo = "$img";
    ...
    $order++;
    $sql = "INSERT INTO iptv_channels (id,name,category_id,tvg_id,logo,stream,ondemand,stream_server,stream_profile,enable,ordering,number) VALUES({$row['id']},'{$name}','$category_id','{$tvg_id}','$logo','$stream','$ondemand',1,1,1,$order,$order) ";
...
$r=$DB_XC->query("SELECT * FROM users_packages ");
while($row=mysqli_fetch_array($r))  {
...
echo  "\nImporting accounts...\n";
$r=$DB_XC->query("SELECT * FROM `lines`");
while($row=mysqli_fetch_array($r)) {

    $trial = 0;
    if($row['is_trial']) $trial = 1;

    $b = serialize(json_decode($row['bouquet'],1));

    $package_id = $row['package_id'];

    $mac='';
    if($row['is_mag']) {
        $r2=$DB_XC->query("SELECT mac FROM mag_devices WHERE user_id = {$row['id']} ");
        $mac= base64_decode(mysqli_fetch_array($r2)['mac']);
    }

    $time = time();
    if($row['exp_date'] != NULL ) $time = $row['exp_date'];
    else $time = time() + ( 60 * 60 * 24 * 365 * 10 );

    $row['username'] = mb_convert_encoding( utf8_encode($row['username']), "UTF-8", "UTF-8");
    $row['username'] = mysqli_real_escape_string($DB,$row['username']);
    $token = mysqli_real_escape_string($DB,$row['password']);

    $sql = "INSERT INTO iptv_devices (id,user_id,name,username,token,limit_connections,pin,demo,status,bouquets,mag,add_date,subcription,package_id) VALUES('{$row['id']}','{$row['member_id']}','{$row['username']}','{$row['username']}','$token','{$row['max_connections']}','0000','$trial','1','$b','$mac',NOW(), DATE_FORMAT(FROM_UNIXTIME($time),'%Y-%m-%d'),'$package_id' ) ";
    if(! $DB->query($sql)) { echo "\n{$RED}Importer have fail.\n\n{$WHITE}"; logErr(); }
...
echo "\nImporting Users...\n";
$r=$DB_XC->query("SELECT * FROM users ");
while($row=mysqli_fetch_array($r)) {
    ...
    //$row['username'] = mb_convert_encoding( utf8_encode($row['username']), "UTF-8", "UTF-8");
    $row['username'] = mysqli_real_escape_string($DB,$row['username']);
    $pass = md5(mysqli_real_escape_string($DB,$row['username']));

    $sql = "INSERT INTO users (id,username,password,email,status,signup_date) VALUES('{$row['id']}','{$row['username']}','$pass','$email',1,DATE_FORMAT(FROM_UNIXTIME($signup_date),'%Y-%m-%d %H:%i:%s')) ";
    if(! $DB->query($sql)) { echo "\n{$RED}Importer have fail.\n\n{$WHITE}"; logErr(); }
    ...
    $sql = "INSERT INTO iptv_user_info (user_id,credits,override_packages) VALUES('{$row['id']}','{$row['credits']}','$op')";
    if(! $DB->query($sql)) { echo "\n{$RED}Importer have fail.\n\n{$WHITE}"; logErr(); }
